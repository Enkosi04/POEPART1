...
class SafeandSuspiciousLinks
{
    public void StartChatbot()
    {
        ...
        // Automatically display the ASCII art after greeting
        DisplayImages(new[] { "SafeandSuspiciouslinks.jpg" });
    ...
    {
        if (input.Contains("How to identify safe and suspicious", StringComparison.OrdinalIgnoreCase))
        {
            return "To identify safe links, hover over them to see the actual URL"+
                "Be wary of shortened URLs and ensure the domain matches the expected site";
        }
        else
        {
    ...
    private void DisplayImages(string[] imagePaths)
    {
        ...
        {
            Console.WriteLine($"Attempting to load image: {imagePath}");
            try
            {
